...
with open('open.html', 'wt') as htmlf:
    htmlf.writelines(df.describe().to_html())

dump(df, '../clean_data/appendicitis.joblib')
for c in df.columns:
    print(df[c].unique())
    ...
    # plt.savefig('../output/appendicitis/bar_' + c + '.png')
    # plt.close()
    
    # plt.figure()
    # plt.title('Distribuição de ' + c)
    # plt.pie(df[c].value_counts(), labels=df[c].unique())
    # plt.savefig('../output/appendicitis/pie_' + c + '.png')
    # plt.close()
    print(df.shape[0])
    print(f'saved {c}')
